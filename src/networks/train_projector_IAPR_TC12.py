...
# 导入 IAPR TC-12 数据集
from src.datasets.preprocess_IAPR_TC12 import IAPRTC12Dataset

# 设置混合精度训练
use_amp = True
# 初始化tensorboard
writer = SummaryWriter(comment="runs/clip_iapr")
# 设置CUDNN
    ...
    for epoch in range(epochs):
        ...
        if val_loader is not None:
            ...
            if avg_val_loss < best_loss:
                ...
                filename = f"{hyper_dict.projector}_clip_{clip_model.visual.output_dim}_iapr.pth"
            ...
            if epoch % 5 == 0 or epoch == epochs - 1:
                now = datetime.datetime.now()
                filename = now.strftime("%Y%m%d_%H%M%S_iapr.pth")
                save_path_file = os.path.join(hyper_dict.save_path, filename)
                torch.save({'visual_projector': combined_model.visual_projector,
                            'text_projector': combined_model.text_projector}, save_path_file)
                print(f"Model saved to {save_path_file}")
...
if __name__ == "__main__":
    # 解析命令行参数
    parser = argparse.ArgumentParser(description="training with IAPR TC-12 dataset")
    ...
    # IAPR TC-12 数据集参数
    parser.add_argument('--iapr_data_dir', type=str, default="./data/iapr_tc12",
                        help="Directory containing the processed IAPR TC-12 dataset")
    ...
    # 创建 IAPR TC-12 数据集
    print("Loading IAPR TC-12 training dataset...")
    train_dataset = IAPRTC12Dataset(
        data_dir=args.iapr_data_dir,
        split='train',
    ...
    # 创建验证集
    print("Loading IAPR TC-12 validation dataset...")
    val_dataset = IAPRTC12Dataset(
        data_dir=args.iapr_data_dir,
        split='validation',
        img_transform=preprocess
    )
