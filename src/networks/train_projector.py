...
class CombinedModel(nn.Module):
    ...
    def get_image_features(self, images):
        return self.visual_projector(self.clip_model.encode_image(images))

    def get_text_features(self, texts):
        return self.text_projector(self.clip_model.encode_text(texts))
...
def evaluate_loader(model, data_loader, temperature, margin):
    ...
    with torch.no_grad():
        ...
        for images, texts in progress:
            ...
            loss, stats = compute_total_loss(img_feats, text_feats, "", temperature, margin)
...
def train(args, train_loader, val_loader, writer):
    ...
    combined_model = CombinedModel(clip_model, args).to(DEVICE)

    if args.projector == "clip":
    ...
    for epoch in range(args.epochs):
        ...
        for step, (images, texts) in enumerate(progress):
            ...
            with torch.cuda.amp.autocast(enabled=USE_AMP):
                ...
                text_feats = combined_model.get_text_features(text_tokens)
    ...
    emb_dim = clip_model.visual.output_dim
    checkpoint_name = projector_checkpoint_name(args.projector, "clip", emb_dim, normalized_mode)
...
def build_dataloaders(args):
    _, preprocess = clip.load(args.backbone, device=DEVICE)
    train_dataset = MSCOCODataset(args.coco_json, args.coco_img_dir, img_transform=preprocess)
    val_dataset = MSCOCODataset(args.coco_val_json, args.coco_val_img_dir, img_transform=preprocess)
    flickr_dataset = F30kCaptionsCap(args.flickr_split, split=args.flickr_eval_split, transform=preprocess)
...
def parse_args():
    parser = argparse.ArgumentParser(description="Pretrain CLIP projector with configurable loss variants")
    ...
    parser.add_argument("--projector", type=str, default="norm",
...
def main():
    ...
    writer = SummaryWriter(log_dir=os.path.join("runs", f"projector_clip_{args.projector}_{variant_label}"))
