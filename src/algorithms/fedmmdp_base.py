...
class EngineBase(object):
    def __init__(self, args, config, class_size, logger, client=-1, dset_name="mm", device='cuda', mlp_local=False, wandb=None):
        ...
        self.model = get_model(config.model, mlp_local)
        self.set_criterion(get_criterion(config.criterion.name,
                                         config.criterion))
        self.cluster_criterion = losses.create('distance_match')
    ...
    def set_dset(self, dset_name, client=-1):
        if dset_name == "mm":
            self.train_loader = self.dataloaders[client]
            self.val_loader = self.test_loaders[client]
        else:
            assert False
