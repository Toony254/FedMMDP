...
class MMFL(object):
    ...
    def train(self, round_n):
        ...
        local_image_model, local_text_model, local_mm_model = [], [], []
        local_anchors = []
        for idx, trainer in enumerate(self.cur_trainers):
            if hasattr(trainer, 'set_global_anchor'):
                trainer.set_global_anchor(getattr(self, 'global_anchor', None))
            model_state, anchor = trainer.train_with_anchor()
            if trainer.dset_name == 'image':
                local_image_model.append(model_state)
            elif trainer.dset_name == 'text':
                local_text_model.append(model_state)
            else:
                local_mm_model.append(model_state)
            local_anchors.append(anchor)
        if local_anchors:
            self.global_anchor = np.mean(local_anchors, axis=0)
        for trainer in self.cur_trainers:
            if hasattr(trainer, 'set_global_anchor'):
                trainer.set_global_anchor(self.global_anchor)
