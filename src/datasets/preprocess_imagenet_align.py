...
# 配置参数
# ============================================
OUTPUT_DIR = Path("preprocessed_imagenet/domain_datasets/ALIGN")
...
# ============================================
logger.info("=" * 70)
logger.info("Step 2: Loading ALIGN model")
logger.info("=" * 70)
...
logger.info("✓ ALIGN model loaded")
...
def process_and_encode_example(example, domain_idx, class_id):
    """
    处理单个样本：提取图像和文本，使用CLIP编码
    """
    try:
        # 处理图像
        img = Image.open(io.BytesIO(example["image"]))
        img = img.convert('RGB')
        img_tensor = align_preprocess(img).unsqueeze(0).to(DEVICE)
        
        # 处理文本
        text_inputs = align_tokenizer(
            example["recaption_short"],
            padding="max_length",
            truncation=True,
        ...
        # ALIGN编码
        with torch.no_grad():
            image_features = align_model.get_image_features(img_tensor)
            text_features = align_model.get_text_features(**text_inputs)
        ...
        # 转换为numpy
        image_emb = image_features.cpu().numpy().squeeze()
        text_emb = text_features.cpu().numpy().squeeze()
        
        return {
            "id": example["id"],
            "recaption_short": example["recaption_short"],
            "class_id": class_id,
            "processed_img": image_emb.tolist(),
            "cap_tokens": text_emb.tolist()
        }
